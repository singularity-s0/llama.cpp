...
static ggml_backend_buffer_type_t llama_default_buffer_type_offload(int main_gpu) {
    ggml_backend_buffer_type_t buft = nullptr;

#ifdef GGML_USE_METAL
    buft = ggml_backend_metal_buffer_type();
#elif defined(GGML_USE_CUBLAS)
    buft = ggml_backend_cuda_buffer_type(main_gpu);
#endif
...
static bool llama_kv_cache_init(
    ...
    struct ggml_context * ctx_cpu = NULL;
    struct ggml_context * ctx_offload = NULL;

    if (!offload || n_gpu_layers < n_layer) {
        int n_cpu_layers = n_layer - n_gpu_layers;
        struct ggml_init_params params;
        params.mem_size   = 2u*n_cpu_layers*ggml_tensor_overhead();
        params.mem_buffer = NULL;
        params.no_alloc   = true;
        ctx_cpu = ggml_init(params);
        if (!ctx_cpu) {
            LLAMA_LOG_ERROR("%s: failed to allocate CPU context for kv cache\n", __func__);
            return false;
        }
        cache.ctxs.push_back(ctx_cpu);
    }

    if (offload && n_gpu_layers > 0) {
        struct ggml_init_params params;
        params.mem_size   = 2u*n_gpu_layers*ggml_tensor_overhead();
        params.mem_buffer = NULL;
        params.no_alloc   = true;
        ctx_offload = ggml_init(params);
        if (!ctx_offload) {
            LLAMA_LOG_ERROR("%s: failed to allocate offload context for kv cache\n", __func__);
            return false;
        }
        cache.ctxs.push_back(ctx_offload);
    }
    ...
    for (int i = 0; i < (int) n_layer; i++) {
        struct ggml_context * ctx = (i >= i_gpu_start && offload) ? ctx_offload : ctx_cpu;
    ...
    if (ctx_cpu) {
        ggml_backend_buffer_t buf_cpu = ggml_backend_alloc_ctx_tensors_from_buft(ctx_cpu, llama_default_buffer_type_cpu(false));
        if (!buf_cpu) {
            LLAMA_LOG_ERROR("%s: failed to allocate CPU buffer for kv cache\n", __func__);
            return false;
        }
        ggml_backend_buffer_clear(buf_cpu, 0);
        // FIXME: buffer type name
        LLAMA_LOG_INFO("%s: KV %10s buffer size: %.02f MiB\n", __func__, "CPU", ggml_backend_buffer_get_size(buf_cpu)/1024.0/1024.0);
        cache.bufs.push_back(buf_cpu);
    }

    if (ctx_offload) {
        ggml_backend_buffer_t buf_offload = ggml_backend_alloc_ctx_tensors_from_buft(ctx_offload, llama_default_buffer_type_offload(main_gpu));
        if (!buf_offload) {
            LLAMA_LOG_ERROR("%s: failed to allocate offload buffer for kv cache\n", __func__);
            return false;
        }
        ggml_backend_buffer_clear(buf_offload, 0);
        // TODO: use correct backend name
        LLAMA_LOG_INFO("%s: KV %10s buffer size: %.02f MiB\n", __func__, "offload", ggml_backend_buffer_get_size(buf_offload)/1024.0/1024.0);
        cache.bufs.push_back(buf_offload);
    }
...
static bool llm_load_tensors(
    ...
    // create the ggml contexts

    // need a separate context for each buffer type

    // at this moment, this is:
    // buffer_type_cpu
    // buffer_type_offload
    // buffer_type_split
    // however, we should be able to add more buffer types in the future, for offloading to different backends, etc
    // therefore, this should be fairly generic
    // std::vector<ggml_context *> ctxs;
    // ctxs.resize(buffer_type_count);

    ggml_context * ctx_cpu;
    ggml_context * ctx_offload;
...

    {
        ...
        if (n_gpu_layers > 0) {
            ...
            model.ctxs.push_back(ctx_offload);

            ctx_split = ggml_init(params);
            if (!ctx_split) {
                throw std::runtime_error(format("ggml_init() failed"));
            }
            model.ctxs.push_back(ctx_split);
        } else {
            ctx_offload = ctx_cpu;
            ctx_split   = ctx_cpu;
        }
    ...
    // create tensors for the weights
    {
        ...
        const int64_t i_gpu_start  = (int64_t) hparams.n_layer - n_gpu_layers;
        ...
        // and it would reduce the metal buffer size
        ggml_context * ctx_input        = ctx_cpu;
        ggml_context * ctx_output       = n_gpu_layers > n_layer ? ctx_offload : ctx_cpu;
        ggml_context * ctx_output_split = n_gpu_layers > n_layer ? ctx_split   : ctx_cpu;
        auto ctx_for_layer              = [=](int i) { return i < i_gpu_start ? ctx_cpu : ctx_offload; };
        auto ctx_for_layer_split        = [=](int i) { return i < i_gpu_start ? ctx_cpu : ctx_split;   };
    ...
    // NOT GOOD! all buffers could be mmap
    // metal with partial offload: both cpu and metal buffers could be mmap
    // we need a flag to indicate whether a buffer is mmap or not
    // we could simply use ggml_backend_buffer_is_host() to check
    // currently this is fine, but future backends need to be aware
    // that all host buffers need to be mmap with a _from_ptr() function
    // however, we need to check the range of the mmaped file that is actually used
    // otherwise, partial offload makes no sense
    // if the whole file can be mmaped to a metal buffer, then we can simply use the metal backend for the whole model
    ggml_backend_buffer_t buf_mmap = nullptr;

#ifdef GGML_USE_METAL
    if (n_gpu_layers > 0) {
        if (ml.use_mmap) {
            const size_t max_size = ggml_get_max_tensor_size(ctx_offload);
            buf_mmap = ggml_backend_metal_buffer_from_ptr(ml.mapping->addr, ml.mapping->size, max_size);
        }
    }
#endif
    // CPU backend
    // FIXME: we should only create the CPU buf_mmap if there there any tensors in the CPU backend
    if (ml.use_mmap && !buf_mmap) {
        buf_mmap = ggml_backend_cpu_buffer_from_ptr(ml.mapping->addr, ml.mapping->size);
    }

    ggml_backend_buffer_t buf_cpu     = nullptr;
    ggml_backend_buffer_t buf_offload = nullptr;
    ggml_backend_buffer_t buf_split   = nullptr;

    // FIXME: avoid creating these buffers if we are using a buf_mmap
    ggml_backend_buffer_type_t buft_cpu     = llama_default_buffer_type_cpu(true);
    ggml_backend_buffer_type_t buft_offload = llama_default_buffer_type_offload(main_gpu);
    ggml_backend_buffer_type_t buft_split   = llama_default_buffer_type_split(main_gpu, tensor_split);
    ...
    // ggml_backend_alloc_ctx_tensors_from_buft() will return NULL both if there are no tensors and if there was an error allocating the buffer
    // so avoid calling it if there are no tensors in the context
    if (ctx_cpu != nullptr && ggml_n_tensors(ctx_cpu) > 0) {
        buf_cpu = ggml_backend_alloc_ctx_tensors_from_buft(ctx_cpu, buft_cpu);
        model.bufs.push_back(buf_cpu);
        if (buf_cpu == nullptr) {
            throw std::runtime_error("failed to allocate CPU buffer");
        }
    }

    if (ctx_offload != ctx_cpu && ctx_offload != nullptr && ggml_n_tensors(ctx_offload) > 0) {
        buf_offload = ggml_backend_alloc_ctx_tensors_from_buft(ctx_offload, buft_offload);
        model.bufs.push_back(buf_offload);
        if (buf_offload == nullptr) {
            throw std::runtime_error("failed to allocate offload buffer");
        }
    }

    if (ctx_split != ctx_cpu && ctx_split != ctx_offload && ctx_split != nullptr && ggml_n_tensors(ctx_split) > 0) {
        buf_split = ggml_backend_alloc_ctx_tensors_from_buft(ctx_split, buft_split);
        model.bufs.push_back(buf_split);
        if (buf_split == nullptr) {
            throw std::runtime_error("failed to allocate split buffer");
        }
    }
    ...
    if (use_mlock) {
        for (ggml_backend_buffer_t buf : model.bufs) {
            ...
            // if (ggml_backend_buffer_type(buf) == ggml_backend_cpu_buffer_type()) {
                // FIXME: does this make sense for metal buffers? they are probably already pinned in newBufferWithBytesNoCopy
    ...
    // print memory requirements
    {
        ...
        for (ggml_backend_buffer_t buf : model.bufs) {
            ...
            if (buf == buf_cpu) {
                name = "CPU";
            } else if (buf == buf_offload) {
                name = "offload";
            } else if (buf == buf_split) {
                name = "split";
            } else {
                name = "unknown";
            }
    ...
    // load data
    for (ggml_context * ctx : model.ctxs) {
        // FIXME: use the correct buf_mmap for the context
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        if (!llama_kv_cache_init(ctx->model.hparams, ctx->kv_self, type_k, type_v,
                cparams.n_ctx, model->main_gpu, model->n_gpu_layers, cparams.offload_kqv)) {
